...
export async function GET() {
  ...
  return ok(await prisma.catalogInjury.findMany({ where: { active: true }, orderBy: { name: "asc" } }));
}
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return badRequest("Lesión inválida.", parsed.error.flatten());

  return created(
    await prisma.catalogInjury.create({
